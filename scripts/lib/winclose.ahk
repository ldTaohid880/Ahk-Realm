    ...
    if (idObject = OBJID_WINDOW && idChild = INDEXID_CONTAINER) { ; Filters out only windows
        ...
        if (event = EVENT_OBJECT_CREATE && DllCall("IsTopLevelWindow", "Ptr", hwnd)) {
            try {
                ...
                gOpenWindows[hwnd] := { title: WinGetTitle(hwnd), class: WinGetClass(hwnd), processName: WinGetProcessName(
                    hwnd) }
                OutputDebug(gOpenWindows[hwnd].processName " <<== New Open")
            }
        } else if (event = EVENT_OBJECT_DESTROY) {
            if gOpenWindows.Has(hwnd) {
                OutputDebug gOpenWindows[hwnd].processName " <<== Closed"
                ; Delete info about windows that have been destroyed to avoid unnecessary memory usage
                gOpenWindows.Delete(hwnd)
            }
        } else if (event = EVENT_SYSTEM_FOREGROUND) {
            OutputDebug(WinGetProcessName(hwnd) ' <<== active')
        } else if (event = EVENT_SYSTEM_MINIMIZESTART) {
            OutputDebug(WinGetProcessName(hwnd) ' <<== minimized')
        } else if (event = EVENT_SYSTEM_MINIMIZEEND) {
            OutputDebug(WinGetProcessName(hwnd) ' <<== maximized')
        } else if (event = EVENT_SYSTEM_MOVESIZEEND) {
            OutputDebug(WinGetProcessName(hwnd) ' <<== moveEnd')
        } else if (event = EVENT_SYSTEM_MOVESIZESTART) {
            OutputDebug(WinGetProcessName(hwnd) ' <<== MoveStart')
        }
